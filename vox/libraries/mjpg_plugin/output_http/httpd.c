...
void send_stream(cfd *context_fd, int input_number, int thumb)
{
    ...
    while(!pglobal->stop) {
        ...
        // kiosk에서의 요청은 160*120 으로 해상도 변경
        {
            ...
            if(thumb)
            {
                ...
                //printf("#1 cm.data.ptr=%p, cm.rows=%d, cm.cols=%d\n", cm.data.ptr, cm.rows, cm.cols);
                ...
                //printf("#2 enc->data.ptr=%p, enc->rows=%d, enc->cols=%d\n", enc->data.ptr, enc->rows, enc->cols);

                memcpy(frame, enc->data.ptr, enc->rows * enc->cols);
                frame_size = enc->rows * enc->cols;
