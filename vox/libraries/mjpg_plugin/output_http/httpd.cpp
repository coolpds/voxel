...
int _read(int fd, iobuffer *iobuf, void *buffer, size_t len, int timeout)
{
    ...
    while((copied < len)) {
        ...
        memcpy((void*)(buffer + copied), (void*)(iobuf->buffer + IO_BUFFER - iobuf->level), i);
...
void send_stream(cfd *context_fd, int input_number, int thumb)
{
    ...
    while(!pglobal->stop) {
        ...
        // opencv 해상도(800*600)와 stream(320*240) 해상도 변경
        {
            // ?action=stream&thumb 로 요청함
            if(thumb)
            {
                ...
                Mat src = imdecode(
                            cv::Mat(1, frame_size, CV_8UC1, frame), 
                ...
                Mat dst;
                resize(src, dst, Size(320, 240), INTER_LINEAR /*INTER_CUBUC*/ /*INTER_LINEAR*/);

                std::vector<unsigned char> outbuf;
                imencode(".jpg", dst, outbuf);

                //printf("outbuf.size()=%zu\n", outbuf.size());

                memcpy(frame, &outbuf[0], outbuf.size());
                frame_size = outbuf.size();

                //printf("fsize(%zd) dst.data=%p, dst.rows=%d, dst.cols=%d\n", frame_size, dst.data, dst.rows, dst.cols);

#if 0
                CvMat cm = cvMat(1, frame_size, CV_8UC1, frame);
                IplImage* src = cvDecodeImage(&cm, CV_LOAD_IMAGE_COLOR);
                if(!src) break;

                IplImage* dst = cvCreateImage(cvSize(160, 120), src->depth, src->nChannels);
                if(!dst) break;
                ...
                //DBG("enc->data.ptr=%p, enc->rows=%d, enc->cols=%d\n", enc->data.ptr, enc->rows, enc->cols);

                memcpy(frame, enc->data.ptr, enc->cols);
                frame_size = enc->cols;
...
void *client_thread(void *arg)
{
    ...
    // 이미지 스케일 조정(chromium kiosk에서 cpu로드가 너무 큼)
    // /?action=stream&thumb
    int thumb = 0;
    ...
    // 스트리밍 해상도에 따른 kiosk 성능 저하 문제
    if(strstr(buffer, "thumb") != NULL)
        thumb = 1;
    ...
    case A_STREAM:
        ...
        send_stream(&lcfd, input_number, thumb);
        break;
    #ifdef WXP_COMPAT
    case A_STREAM_WXP:
